...
//			System.out.println("ip: "+onefw.getdestip()+" length: "+ onefw.getprefixlen());
//			aclBDD.printDot("FwdBDD", entrybdd);
		...
		/*
		 * test <port, BDD> for each device
		 */
		
//		for(String portNum : fwdbdds.keySet()){
//			System.out.println("portName: "+ portNum + " BDD:  " + fwdbdds.get(portNum) );
//		}
//		System.out.println("sizeOf: "+ fwdbdds.size() );
	 ...
	 * @return a bdd node that represents the acl w.r.t. accept predicate
	 */
	public int ConvertACLs(LinkedList<ACLRule> acls)
	{
		...
		int res = BDDFalse; //accept predicate
	 ...
	 * 
	 * @param r - the range
	 * @param vars - bdd variables used, stored in a int array
	 * @param bits - number of bits in the representation
	 * @return the corresponding bdd node
	 */
	private int ConvertRange(Range r, int [] vars, int bits)
	{
		/*convert a range to a prefix, eg.: when bits = 4, 10 = 10xx*/
